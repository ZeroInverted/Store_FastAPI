...
@product_router.get("/", response_model=Product_API_Response)
...
@product_router.get("/{id}", response_model=Product_API_Response)
def get_specific_product(id: int, db: Session = Depends(get_db)):
    return get_product_by_id(db, id)
    
@product_router.post("/", response_model=Product_API_Response)
def post_product(product: ProductInput, db: Session = Depends(get_db)):
    return create_product(db, product)

@product_router.put("/{id}", response_model=Product_API_Response)
